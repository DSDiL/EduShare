...
class _DonFormState extends State<DonForm> {
  ...
  Future<List<InterestAreaModel>> GetInterestAres() async {
    ...
    CollectionReference interestAreas =
        FirebaseFirestore.instance.collection('interestAreas');
    ...
    for (var doc in querySnapshot.docs) {
      InterestAreaModel area =
          new InterestAreaModel(doc['name'], doc['description']);
      Areas.add(area);
    }
    return Areas;
  }
  ...
  Future submitForm() async {
    ...
    if (isAccountFormValid) {
      ...
      DonPostModel model = new DonPostModel(
          userAvatar: userController.getUser.imgUrl,
          thumbnailUrl: urlUpload,
          contactEmail: contactEmail.text.trim(),
          createdAt: DateTime.now(),
          createdBy: FirebaseAuth.instance.currentUser?.uid,
          details: details.text,
          interest: targetAreas,
          phoneNo: phoneNo.text.trim(),
          searchTag: title.text.toLowerCase().trim(),
  ...
  Future updateForm() async {
    ...
    if (isAccountFormValid) {
      ...
      DonPostModel model = new DonPostModel(
          userAvatar: userController.getUser.imgUrl,
          createdAt: widget.updateModel!.createdAt!,
          thumbnailUrl: urlUpload.length != 0
              ? urlUpload
              : widget.updateModel!.thumbnailUrl!,
          contactEmail: contactEmail.text.trim(),
          createdBy: FirebaseAuth.instance.currentUser?.uid,
          details: details.text,
          interest: targetAreas,
                    ...
                    children: [
                      TextFormField(
                          ...
                          decoration: InputDecoration(
                              labelText: 'Post Title')), //Post Title
                      TextFormField(
                          ...
                          controller: details,
                          decoration: InputDecoration(
                              labelText: 'Post Details')), //Post Details
                      Container(
                        height: 400.h,
                        child: FutureBuilder(
                            future: GetInterestAres(),
                            builder: (context,
                                AsyncSnapshot<List<InterestAreaModel>>
                                    snapshot) {
                              if (snapshot.hasData) {
                      ...
                      TextFormField(
                          ...
                          controller: phoneNo,
                          decoration: InputDecoration(
                              labelText: 'Contact No')), //contact No
                      TextFormField(
                          ...
                          decoration: InputDecoration(
                              labelText: 'Contact Email')), //contactEmail
                      ...
                      isUpdating
                          ? CustomUploadImage(
                              ...
                              url: widget.updateModel!.thumbnailUrl!)
                          : CustomUploadImage(
                              onPressed: getImage, imageFile: _image),
                      SizedBox(height: 20.h),
                      isUpdating
                          ? CustomLoadingButton(
  ...
  List<Widget> getChipWidget(List<InterestAreaModel> list) {
    return list
        .map((e) => FilterChip(
              ...
              selectedColor: tPrimaryColor.withOpacity(0.25),
            ))
        .toList();
  }
}
